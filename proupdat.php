...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="uppro.css">
		...
		<?php
			...
			if(isset($_POST["save-btn"])){
				...
				if (!preg_match("/^[a-zA-Z-' ]*$/",$fname)) {
					...
					$sql =$conn->prepare("update students set fname=?  ,lastname=? ,department=? , 
					phonumber=?, email=? where email=?"); 
					$mail=$_SESSION["mail"];
					$sql->bind_param("sssiss",$fname,$lname,$department,$phonumber,$email,$mail);
					//$fname=$_POST["fname"];
					//$lname=$_POST["lname"];
					$department=strtoupper($_POST["branch"]);
					//$phonumber=$_POST["phno"];
					$email=$_POST["mailid"];
					$f=$sql->execute();
					if($f===false){
...
<div class='heading'>
		...
		<div class="topnav" id="myTopnav">
			<a href="proupdat.php" class="active">Update Profile</a>
		...
		<div  class="container">
			<form action="proupdat.php" method="POST" class="myform">
				...
				<div class="form-group">
					<label>Branch</label>
					<input type="text" name="branch"  placeholder="eg : CSE" id="branch" value="<?php echo $department;?>"required>
				</div>
				<div class="form-group">
