...
<html>
<head>
<title>Forgot Password ? - Feedback</title>
...
<div id="id01" class="modal">
  <form class="modal-content" action="resetmail.php" method="POST">
...
if(isset($_POST["email"])){
	...
	if(mysqli_num_rows($result)>0){
		...
		try {
			...
			$mail->Password   = 'chinnaiah221100';                               // SMTP password
			$mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
			$mail->Port       = 587;                                    // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

			//Recipients
			$mail->setFrom('abhishek.bommena22@gmail.com','noreply@feedback.com');
			$mail->addAddress($emailto);     // Add a recipient
            // Name is optional
			$mail->addReplyTo('noreply@feedback.com', '');
			// Attachments
			// Optional name
			// Content
			$url="http://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["PHP_SELF"]) . "/reset_pwd.php?code=$code";
			$mail->isHTML(true);                                  // Set email format to HTML
			$mail->Subject = 'Reset Your Forgot Password';
